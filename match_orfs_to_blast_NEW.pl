...
while(<$file>){
		...
		if($hit_truelen > 4500){
			if($hit_curlen/$hit_truelen >= 0.20){
...
sub match_end{
	...
	my $tempend;
	my $seq = $_[0];
	$tempend = index($seq,substr($_[1],-3),$_[2]*3);
=item	
	for (my $i=0; $i <$i>($_[2]*3);$i){
		if(index($seq,substr($_[1],-3),$i)>=0){#second parameter is gene_seq
			if(index($seq,substr($_[1],-3),$i) < length($seq)){
				$tempend =index($seq,substr($_[1],-3),$i);
				$i=$tempend+1;
			}
			else{
				$i++;
...
=cut	
	$tempend = length($seq)-($tempend + 2)-1;
	return($tempend);
...
sub exon_mods_start {
	...
	if($formatch>$revmatch){
		...
		for (my $i=0; $i<(length($orf_prot)-1);$i++){
		...
		if(substr($gene_prot, -1) eq "_"){
			...
			$end_match = &match_end($orf_seq,$gene_seq,$nearend);
		}
		...
		if($start_match == -1 && $start_alt == -1){
			$final_annotation{$idorfs_f{$gid}{$eid}{"Start"}+($nearstart*3)-3+1}{$idorfs_f{$gid}{$eid}{"End"}-$end_match+$frame_restore+1}{"+"}=$gid;		}
		elsif($start_match>=$start_alt){
			$final_annotation{$idorfs_f{$gid}{$eid}{"Start"}+$start_match-$for_frame+1}{$idorfs_f{$gid}{$eid}{"End"}-$end_match+$frame_restore+1}{"+"}=$gid;
		}
		else{
			$final_annotation{$idorfs_f{$gid}{$eid}{"Start"}+$start_alt-$for_frame+1}{$idorfs_f{$gid}{$eid}{"End"}-$end_match+$frame_restore+1}{"+"}=$gid;
		}
	}
		...
		$nearstart = $nearstart-$fix_nearstart;
		my $nearend;
		for (my $i=0; $i<(length($rev_orf_prot)-1);$i++){
		if(index($rev_orf_prot,substr(substr($gene_prot,-4),0,3),$i)>=0){#third parameter is gene_seq
			if(index($rev_orf_prot,substr(substr($gene_prot,-4),0,3),$i) < (length($rev_orf_prot))){
				$nearend =index($rev_orf_prot,substr(substr($gene_prot,-4),0,3),$i);
				$i=$nearend;
			}
		...
		if(substr($gene_prot, -1) eq "_"){
			...
			$end_match = &match_end($rev_orf_seq,$gene_seq, $nearend);
		}
		...
		if($start_match == -1 && $start_alt == -1){
			$final_annotation{$idorfs_f{$gid}{$eid}{"Start"}+$end_match-$frame_restore+1}{$idorfs_f{$gid}{$eid}{"End"}-($nearstart*3)+3+1}{"-"}=$gid;
		}
		elsif($start_match>$start_alt){
			$final_annotation{$idorfs_f{$gid}{$eid}{"Start"}+$end_match-$frame_restore+1}{$idorfs_f{$gid}{$eid}{"End"}-$start_match-$rev_frame+1}{"-"}=$gid;
		}
		else{
			$final_annotation{$idorfs_f{$gid}{$eid}{"Start"}+$end_match-$frame_restore+1}{$idorfs_f{$gid}{$eid}{"End"}-$start_alt-$rev_frame+1}{"-"}=$gid;
		}
	}
...
sub exon_mods_end {
	...
	if($formatch>$revmatch){
		...
		if(substr($gene_prot, -1) eq "_"){
			...
			$end_match = &match_end($rev_orf_seq,$gene_seq, $nearstart);
		}
		my $start_alt = &alt_start($rev_orf_seq,$nearstart,$gene_seq);
...
sub exon_mods_mid{
	...
	if($formatch>$revmatch){
		...
		my $start_alt = &alt_start($orf_seq,$nearstart,$gene_seq);
		my $end_match = &match_end($orf_seq,$gene_seq,$nearend);
		...
		for (my $i=0; $i<(length($rev_orf_prot)-1);$i++){
			if(index($rev_orf_prot,substr(substr($gene_prot,-4),0,3),$i)>=0){#third parameter is gene_seq
				if(index($rev_orf_prot,substr(substr($gene_prot,-4),0,3),$i) < (length($orf_prot))){
					$nearend =index($rev_orf_prot,substr(substr($gene_prot,-4),0,3),$i);
					$i=$nearend+1;
				}
		...
		my $start_alt = &alt_start($rev_orf_seq,$nearstart,$gene_seq);
		my $end_match = &match_end($rev_orf_seq,$gene_seq,$nearend);
