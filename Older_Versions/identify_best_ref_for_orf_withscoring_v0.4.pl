...
my $current_hsp_query;
my $current_hsp_subject;
...
while(<$file>){
		...
		if(exists $blast_gene_hsps{$tarray[0]}){
			...
			if($current_hsp_query){
				my $hit_length=0;
				for my $tempi (keys %{$blast_gene_hsps{$current_hsp_query}{$current_hsp_subject}}){
					$hit_length+=$blast_gene_hsps{$current_hsp_query}{$current_hsp_subject}{$tempi};
				}
				my $templen = length($blastgenes{$current_hsp_subject});
				if($hit_length/$templen < 0.9){
					delete $blast_gene_hsps{$current_hsp_query}{$current_hsp_subject};
				}
			}	
			for (my $i=0; $i<=length($blastgenes{$tarray[1]})-1; $i++){
				$blast_gene_hsps{$tarray[0]}{$tarray[1]}{$i}=0;
